...
class TestGetFreelancerProfileUseCase:
    async def test_returns_profile(self, profile_repo, make_profile):
        await make_profile(profile_id="profile-1", user_id="user-1", city="Tehran")
        use_case = GetFreelancerProfileUseCase(profile_repo=profile_repo)

        result = await use_case.execute(GetFreelancerProfileQuery(profile_id="profile-1"))
