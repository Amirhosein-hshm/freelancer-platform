...
class TestSubmitFreelancerApprovalUseCase:
    async def test_pending_profile_can_submit(self, profile_repo, uow, make_profile):
        await make_profile(user_id="user-1")
        use_case = build_use_case(profile_repo, uow)

        result = await use_case.execute(SubmitFreelancerApprovalCommand(user_id="user-1"))

        assert result.profile_id == "profile-1"
        assert result.approval_status == FreelancerApprovalStatus.PENDING
        assert uow.committed is True

    async def test_rejected_profile_resubmits(self, profile_repo, uow, make_profile):
        await make_profile(user_id="user-1", approval_status=FreelancerApprovalStatus.REJECTED)
        use_case = build_use_case(profile_repo, uow)
    ...
    async def test_approved_profile_cannot_submit(self, profile_repo, uow, make_profile):
        await make_profile(user_id="user-1", approval_status=FreelancerApprovalStatus.APPROVED)
        use_case = build_use_case(profile_repo, uow)

        with pytest.raises(InvalidStateTransitionError):
            await use_case.execute(SubmitFreelancerApprovalCommand(user_id="user-1"))
