...
class TestAcceptFreelancerUseCase:
    async def test_accept_selection_and_rejects_others(
        self,
        ...
        await add_application(application_repo, "app-1", "profile-1", now=await clock.now())
        await add_application(application_repo, "app-2", "profile-2", now=await clock.now())
        ...
        result = await use_case.execute(AcceptFreelancerCommand(actor_id="customer-1", application_id="app-1"))
        ...
        assert (await application_repo.get_by_id("app-1")).status == ProjectApplicationStatus.ACCEPTED
        assert (await application_repo.get_by_id("app-2")).status == ProjectApplicationStatus.REJECTED
        assert len(await status_history_repo.list_by_project("project-1")) == 1
    ...
    async def test_non_owner_raises(
        ...
        with pytest.raises(PermissionDeniedError):
            await use_case.execute(AcceptFreelancerCommand(actor_id="intruder", application_id="app-1"))


class TestRejectFreelancerUseCase:
    async def test_reject_application(
        self, authorization_service, project_repo, application_repo, clock, uow, make_project
    ):
        authorization_service.grant("customer-1", "project.manage_own")
        await make_project(project_id="project-1", customer_user_id="customer-1")
        await add_application(application_repo, "app-1", "profile-1", now=await clock.now())
        ...
        assert (await application_repo.get_by_id("app-1")).decision_note == "No fit"
...
class TestViewApplicationsUseCase:
    async def test_view_applications_as_customer(
        ...
        result = await use_case.execute(ViewApplicationsQuery(actor_id="customer-1", project_id="project-1"))

        assert len(result.applications) == 2

    async def test_non_owner_raises(self, authorization_service, project_repo, application_repo, make_project):
        await make_project(project_id="project-1", customer_user_id="customer-1")
        use_case = ViewApplicationsUseCase(
            authorization_service=authorization_service,
            project_repo=project_repo,
        ...
        with pytest.raises(PermissionDeniedError):
            await use_case.execute(ViewApplicationsQuery(actor_id="intruder", project_id="project-1"))
