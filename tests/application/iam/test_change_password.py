...
class TestChangePasswordUseCase:
    async def test_change_password_succeeds(self, user_repo, password_hasher, clock, uow, make_user):
        await make_user(user_id="u1", password="old-pass")
        ...
        assert result.password_changed_at == await clock.now()
    ...
    async def test_change_password_missing_fields_raises_validation(
        ...
        with pytest.raises(ValidationError):
            await use_case.execute(ChangePasswordCommand(user_id="u1", old_password="", new_password=""))
