...
class TestAdminListUsersUseCase:
    ...
    async def test_unfiltered_list_returns_all_with_total(
        self, authorization_service, user_repo, make_user
    ):
        ...
        result = await use_case.execute(
            AdminListUsersQuery(actor_id="admin", page=1, page_size=2)
        )
    ...
    async def test_unfiltered_list_second_page(
        self, authorization_service, user_repo, make_user
    ):
        authorization_service.grant("admin", "user.read")
        await make_user(user_id="u1")
        ...
        result = await use_case.execute(
            AdminListUsersQuery(actor_id="admin", page=2, page_size=2)
        )

        assert [u.user_id for u in result.users] == ["u3"]
        assert result.total_items == 3

    async def test_status_filter_returns_matching_users_only(
        self, authorization_service, user_repo, make_user
    ):
        authorization_service.grant("admin", "user.read")
        await make_user(user_id="u1")
        ...
        result = await use_case.execute(
            AdminListUsersQuery(actor_id="admin", status=UserStatus.BLOCKED)
        )
