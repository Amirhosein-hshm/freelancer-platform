...
def build_use_case(
    authorization_service, category_repo, project_repo, clock, uow
...
class TestDeleteCategoryUseCase:
    async def test_delete_soft_deletes_category(
        ...
        result = await use_case.execute(
            DeleteCategoryCommand(actor_id="admin", category_id="cat-1")
        )
    ...
    async def test_delete_unknown_category_raises(
        self, authorization_service, category_repo, project_repo, clock, uow
    ):
        authorization_service.grant("admin", "category.manage")
        use_case = build_use_case(
            authorization_service, category_repo, project_repo, clock, uow
        ...
        with pytest.raises(CategoryNotFoundError):
            await use_case.execute(
                DeleteCategoryCommand(actor_id="admin", category_id="ghost")
            )
    ...
    async def test_delete_category_with_children_is_blocked(
        ...
        await make_category(
            category_id="cat-child", parent_category_id="cat-parent", slug="child"
        )
        use_case = build_use_case(
            authorization_service, category_repo, project_repo, clock, uow
        )
        ...
        with pytest.raises(CategoryHasActiveReferencesError) as exc:
            await use_case.execute(
                DeleteCategoryCommand(actor_id="admin", category_id="cat-parent")
            )
    ...
    async def test_delete_category_with_active_projects_is_blocked(
        ...
        await make_category(category_id="cat-1")
        await make_project(
            project_id="proj-1", category_id="cat-1", status=ProjectStatus.PUBLISHED
        )
        use_case = build_use_case(
            authorization_service, category_repo, project_repo, clock, uow
        )
    ...
    async def test_delete_category_with_terminal_project_allowed(
        ...
        await make_category(category_id="cat-1")
        await make_project(
            project_id="proj-1", category_id="cat-1", status=ProjectStatus.COMPLETED
        )
        use_case = build_use_case(
            authorization_service, category_repo, project_repo, clock, uow
        )
