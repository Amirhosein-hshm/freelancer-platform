...
def build_use_case(authorization_service, category_repo, uow) -> UpdateCategoryUseCase:
    return UpdateCategoryUseCase(authorization_service=authorization_service, category_repo=category_repo, uow=uow)
...
class TestUpdateCategoryUseCase:
    async def test_update_category_succeeds(self, authorization_service, category_repo, uow, make_category):
        ...
        result = await use_case.execute(
            UpdateCategoryCommand(actor_id="admin", category_id="cat-1", name="New Name", slug="new-slug")
        )
    ...
    async def test_update_duplicate_slug_raises(self, authorization_service, category_repo, uow, make_category):
        authorization_service.grant("admin", "category.manage")
        ...
        with pytest.raises(DuplicateCategorySlugError):
            await use_case.execute(UpdateCategoryCommand(actor_id="admin", category_id="cat-1", name="X", slug="taken"))
    ...
    async def test_update_unknown_category_raises(self, authorization_service, category_repo, uow):
        ...
        with pytest.raises(CategoryNotFoundError):
            await use_case.execute(UpdateCategoryCommand(actor_id="admin", category_id="ghost", name="X", slug="x"))
    ...
    async def test_update_missing_fields_raises_validation(
        ...
        with pytest.raises(ValidationError):
            await use_case.execute(UpdateCategoryCommand(actor_id="admin", category_id="cat-1", name="", slug=""))
