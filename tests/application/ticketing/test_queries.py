...
class TestGetUserTicketsUseCase:
    async def test_lists_created_and_assigned(self, authorization_service, ticket_repo, make_ticket):
        await make_ticket(ticket_id="ticket-1", created_by="user-1")
...
class TestGetTicketMessagesUseCase:
    async def test_participant_can_read_messages(
        self,
        ticket_repo,
        message_repo,
        participant_repo,
