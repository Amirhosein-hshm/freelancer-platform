...
class FakeTicketMessageRepository(ITicketMessageRepository):
    """Mirrors the SQLAlchemy repository: list reads exclude soft-deleted messages, while
    ``get_by_id`` deliberately does not (the mutation use cases rely on the entity guards)."""
    ...
    async def list_by_ticket(self, ticket_id: EntityId) -> list[TicketMessage]:
        return sorted(
            (m for m in self._store if m.ticket_id == ticket_id and m.deleted_at is None),
            key=lambda m: m.sent_at,
        )

    async def list_by_file_asset_id(self, file_asset_id: EntityId) -> list[TicketMessage]:
        return [
            m for m in self._store if file_asset_id in m.attachment_file_asset_ids and m.deleted_at is None
        ]
