...
class GetFreelancerRatingsUseCase(
    UseCase[GetFreelancerRatingsQuery, GetFreelancerRatingsResult]
):
    def __init__(self, rating_repo: IRatingRepository) -> None:
        self._rating_repo = rating_repo

    def execute(self, request: GetFreelancerRatingsQuery) -> GetFreelancerRatingsResult:
        ratings = self._rating_repo.list_by_freelancer(request.freelancer_profile_id)
        average = self._rating_repo.average_score_for_freelancer(
