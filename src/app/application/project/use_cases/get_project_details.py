...
from app.application.project.permissions import (
    ...
    PERMISSION_PROJECT_READ_PUBLIC,
)
...
class GetProjectDetailsUseCase(UseCase[GetProjectDetailsQuery, GetProjectDetailsResult]):
    ...
    async def execute(self, request: GetProjectDetailsQuery) -> GetProjectDetailsResult:
        ...
        if await self._authorization_service.has_permission(request.actor_id, PERMISSION_PROJECT_MANAGE_ANY):
            ...
            and project.status in (ProjectStatus.PUBLISHED, ProjectStatus.COLLECTING_APPLICATIONS)
            and await self._authorization_service.has_permission(request.actor_id, PERMISSION_PROJECT_READ_PUBLIC)
        ):
            allowed = True
        else:
            allowed = False
            ...
            if not allowed and effective is not None and effective.user.user_id == request.actor_id:
                allowed = True
        if not allowed:
        ...
        project_result = to_project_result(project)
        project_result = replace(project_result, supervisor=effective.user if effective else None)
